...
class Asset(Workflow, ModelSQL, ModelView):
    ...
    depreciation_method = fields.Selection([
            ('linear', 'Linear'),
            ('written down value (wdv)','Written Down Value (WDV)')
            ], 'Depreciation Method',
        states={
            'readonly': (Eval('lines', [0]) | (Eval('state') != 'draft')),
            },
    ...
    def compute_depreciation(self, amount, date, dates,my_amt):
        """
        ...
        if self.depreciation_method == 'written down value (wdv)':
            depreciation = 0
            start_date = max([self.start_date
                    - relativedelta.relativedelta(days=1)]
                + [l.date for l in self.lines])
            ...
            last_ratio = (
                Decimal(min(last_delta.days, last_ndays))
                / Decimal(last_ndays))
            x2 = self.end_date-self.start_date
            x3 = x2.days
            x4 = int(x3/365)
            dep_rate = 1 - float(self.residual_value/self.value)**float(1/x4)
            print("rate:"+str(dep_rate))
            
            if date == dates[0]:
                depreciation *= first_ratio
                print("firstdep:"+str(depreciation))
            else:
                # b=amt*dep_rate
                # c=amt-b
                # amt=c
                # b=0
                # c=0
                # depreciation=int(amt)
                depreciation1 = dep_rate*float(my_amt)
                depreciation = Decimal(depreciation1)
                print("depre:"+str(depreciation))
    ...
    def depreciate(self):
        ...
        asset_line = None
        for date in dates:
            depreciation = self.compute_depreciation(amount, date, dates,my_amt)
            my_amt -= depreciation
            print("innermyamt:"+str(my_amt))
            print("depreciation"+str(depreciation))
